...
def main_worker(gpu, ngpus_per_node, args):
    ...
    modelG, modelD_3d, modelD_3d_R = create_model(args)
    ...
    x = torch.FloatTensor(args.batchSize, args.n_frames_G-1, args.nc,
    ...
    for epoch in range(args.load_pretrain_epoch + 1, args.total_epoch):
        ...
        for step, data in enumerate(dataset):
            ...
            total_steps += 1
            loss_all, loss_names = train_func.GD_step(args, modelG, modelD_3d, modelD_3d_R, data, x, z)
            ...
            if total_steps % args.save_latest_freq == 0 and (
                ...
                print('saving the latest model (epoch %d, total_steps %d)' %
                      (epoch, total_steps))
                save_models(modelG, modelD_3d, modelD_3d_R, args.checkpoints_dir, 'latest')
        ...
        if epoch % args.save_epoch_freq == 0 and (
            ...
            print('saving the model at the end of epoch %d, iters %d' %
                  (epoch, total_steps))
            save_models(modelG, modelD_3d, modelD_3d_R, args.checkpoints_dir, 'epoch_' + str(epoch))
...
def save_models(modelG, modelD_3d, modelD_3d_R, ckpt_dir, string):
    ...
    torch.save(modelD_3d_R.state_dict(),
               '%s/modelD_3d_R_%s.pth' % (ckpt_dir, string))

def save_videos(writer, z, modelG, string, args):
    with torch.no_grad():
        modelG.eval()
        x_fake = modelG(z, args.n_frames_G)
        x_fake = x_fake.view(args.batchSize, (args.n_frames_G-1)*args.interpolation_frame_rate, args.nc,
